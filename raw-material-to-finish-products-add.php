...
if ($submit == 'Add' or $submit == 'Update') {
  $commonQry  = "`productId`='$productId', `quantity`='$quantity', `batchNo`='$batchNo', `activeStatus`='$activeStatus', `packagingDate`='$packagingDate', `inventoryType` = '1', `branchID`='$BranchID'";
  ...
  if ($submit      == 'Add') {
    ...
    $msg        = 'Raw Material To Finish Products Added successfully.';
    if ($success) {
      $description = "Raw Material to Finish Products $name is Added created by $guserName on" . date("l jS \of F Y h:i:s A");
      $logQuery      = "INSERT INTO crm_logtable SET `idType`='5', `useId`='$id', `name`= 'Product $name is Added to Finish Products', `description` ='$description', `createdId`='$guserId', `createdDate`=NOW()";
    ...
    if ($errmsg == '') {
      ...
      $success    = mysqli_query($Dbconnect, $query);
      $msg        = 'Raw Material To Finish Products Updated successfully.';
      if ($success) {
        $description = "Raw Material to Finish Products $name is Updated by $guserName on" . date("l jS \of F Y h:i:s A");
        $logQuery      = "INSERT INTO crm_logtable SET `idType`='5', `useId`='$id', `name`= 'Raw Material to Finish Products $name is Updated', `description` ='$description', `createdId`='$guserId', `createdDate`=NOW()";
        $success    = mysqli_query($Dbconnect, $logQuery);
        location("$currentPage.php?msg=$msg");
      } else {
        $errmsg   = "Error: " . mysqli_error($Dbconnect);
              ...
              <form name="frm" method="post" action="<?php echo $currentPage; ?>-add.php" autocomplete=off>
                <div class="grid grid-cols-12 gap-1.5">
                  <div class="col-span-12 sm:col-span-6 my-1">
                    ...
                    displaySelect('productId', $productsArray, $productId, 'required class="form-control"'); ?>
                  </div>
                  <div class="col-span-12 sm:col-span-6 my-1">
                  ...
                  <div class="col-span-12 sm:col-span-6 my-1">
                    ...
                    <input class="form-control" type="text" id="batchNo" name="batchNo" value="<?php echo $batchNo; ?>" required>
                  </div>
