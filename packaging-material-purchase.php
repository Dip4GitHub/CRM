...
$currentPage    = 'packaging-material-purchase';
            ...
            <div class="card-body">
              ...
              <table class="table table-hover progress-table mt-2">
                ...
                <tbody>
                  <?php
                  // BOF :: Pagination 
                  $query      = $Dbconnect->query("SELECT MAX(A.branchId) AS branchId, MAX(A.vendorName) AS vendorName, MAX(A.invoiceType) AS invoiceType, sum(D.discountAmt) AS discountAmt, sum(D.amount) AS amount, sum(D.finalAmount) AS finalAmount FROM $tableName AS A INNER JOIN `crm_invoice_purchase_details` AS D ON A.id = D.invoiceId WHERE invoiceType = 3 $whrQry GROUP BY A.id ");
