...
if ($submit == 'Add' or $submit == 'Update') {
  $gstAmount = $salePrice - $price;
  $commonQry  = "`name`='$name', `salePrice`='$salePrice', `unit`='$unit', `gstType`='$gstType', `activeStatus`='$activeStatus', `barcode`='$barcode', `hsncode`='$hsncode', `gstAmount`='$gstAmount', `gst`='$gst', `price`='$price'";
  ...
  if ($submit      == 'Add') {
    ...
    if ($success) {
      $description = "Product $name is created by $guserName on" . date("l jS \of F Y h:i:s A");
    ...
    if ($errmsg == '') {
      ...
      if ($success) {
        $description = "Product $name is Updated by $guserName on" . date("l jS \of F Y h:i:s A");
        $logQuery      = "INSERT INTO crm_logtable SET `idType`='5', `useId`='$id', `name`= 'Product $name is Updated', `description` ='$description', `createdId`='$guserId', `createdDate`=NOW()";
        $success    = mysqli_query($Dbconnect, $logQuery);
        location("$currentPage.php?msg=$msg");
      } else {
        $errmsg   = "Error: " . mysqli_error($Dbconnect);
...
<body>
              ...
              <form name="frm" method="post" action="<?php echo $currentPage; ?>-add.php" autocomplete=off>
                <div class="grid grid-cols-12 gap-1.5">
                  ...
                  <div class="col-span-12 sm:col-span-6 my-1">
                    ...
                    <input class="form-control" type="text" id="barcode" name="barcode" placeholder="barcode" value="<?php echo $barcode; ?>">
                  </div>
                  ...
                  <div class="col-span-12 sm:col-span-6 my-1">
                    <label for="salePrice" class="col-form-label">Saleing Price</label>
                    <input class="form-control" type="text" id="salePrice" name="salePrice" readonly="readonly" value="<?php echo $salePrice; ?>" required>
                  </div>
  ...
  <script>
    function getSalePrice(gstType) {
      var gst = parseFloat($('#gst').val());
      var price = parseFloat($('#price').val());
      var gstAmmount = (gst * price) / 100;
      var salePrice = gstAmmount + price;
      if (gstType == 1) {
