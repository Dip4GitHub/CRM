...
$currentPage    = 'products-inventory-report';
$pageTitle       = 'Products Inventory Report';
...
<body>
            ...
            <div class="card-body">
              ...
              <table class="table table-hover progress-table mt-2">
                ...
                <tbody>
                  <?php
                  // BOF :: Pagination 
                  $query      = $Dbconnect->query("SELECT id FROM $tableName WHERE `inventoryType`='6' AND `branchID`='$BranchID' $whrQry");
                  $totalRows  = $query->num_rows;
                  $pagination =  new Pagination(array('baseURL' => $currentPage . '.php', 'totalRows' => $totalRows, 'perPage' => $limitperpage));
                  // EOF :: Pagination 

                  $sql    = generateSql("SELECT *,DATE_FORMAT(packagingDate, '%d-%m-%Y')  AS packagingDate FROM `$tableName` WHERE `inventoryType`='6' AND `branchID`='$BranchID' $whrQry");
                  ...
                  ?>
                      <tr>
                        ...
                        <td><?php if ($result->activeStatus == 2) { ?> <button type="button" class="mx-2 bg-theme-bg-1 btn text-white btn-flat btn-xs" onclick="productStatus(<?php echo $result->id ?>)">Get Recieved</button><?php  } ?></td>
  ...
  <script>
    function productStatus(id) {
      ...
      $.post('includes/ajax/productStatus.php', {
          id: id
        },
