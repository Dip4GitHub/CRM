...
if ($keyword != '') {
  $whrQry = "AND customerName LIKE '%$keyword%'";
}
...
if ($submit      == 'delete') {
  ...
  if ($success) {
    $description = "Purchase/Order #$id From Vendors $vendorName is deleted by $guserName on" . date("l jS \of F Y h:i:s A");
    $logQuery      = "INSERT INTO crm_logtable SET `idType`='7', `useId`='$id', `name`= 'Purchase/Order of Vendors $vendorName is deleted', `description` ='$description', `createdId`='$guserId', `createdDate`=NOW()";
            ...
            <div class="card-body">
              <form name="frm" method="GET" autocomplete=off>
                <div class="search-container">
                  <input type="search" class="keyword" name="keyword" placeholder="Search By Vendors Name" value="<?php echo $keyword; ?>">
              ...
              <table class="table table-hover progress-table mt-2">
                ...
                <tbody>
                  <?php
                  // BOF :: Pagination 
                  $query      = $Dbconnect->query("SELECT MAX(A.branchId) AS branchId, MAX(A.vendorName) AS vendorName, MAX(A.invoiceType) AS invoiceType, sum(D.discountAmt) AS discountAmt, sum(D.amount) AS amount, sum(D.finalAmount) AS finalAmount FROM $tableName AS A INNER JOIN `crm_invoice_purchase_details` AS D ON A.id = D.invoiceId WHERE invoiceType = 2 $whrQry GROUP BY A.id ");
                  ...
                  ?>
                      <tr>
                        ...
                        <!--<td><a href="<?php // echo $currentPage; 
                                          ?>-edit.php?id=<?php // echo $result->id; 
                                                          ?>" class="text-secondary"><span class="pc-micon"> <i data-feather="edit"></i></span></a></td>
                        <td><a href="#" onClick="return deleteRecord('<?php // echo $result->id; 
                                                                      ?>');" class="text-danger"><span class="pc-micon"> <i data-feather="trash-2"></i></span></a></td>-->
