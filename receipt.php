            ...
            <div class="card-body">
              ...
              <table class="table table-hover progress-table mt-2">
                ...
                <tbody>
                  <?php
                  // BOF :: Pagination 
                  $query      = $Dbconnect->query("SELECT I.id, I.branchId, I.customerName, I.invoiceType, I.createdDate, D.discountAmt, D.finalAmount FROM $tableName AS I INNER JOIN crm_invoice_purchase_details AS D ON I.id = D.invoiceId  $whrQry GROUP BY I.id ORDER BY I.id");
                  $totalRows  = $query->num_rows;
                  $pagination =  new Pagination(array('baseURL' => $currentPage . '.php', 'totalRows' => $totalRows, 'perPage' => $limitperpage));
                  // EOF :: Pagination 

                  $i = 1;
                  $sql    = generateSql("SELECT I.id, I.branchId, I.customerName, I.invoiceType, DATE_FORMAT(I.createdDate, '%d-%m-%Y') AS createdDate, D.discountAmt, D.finalAmount FROM $tableName AS I INNER JOIN crm_invoice_purchase_details AS D ON I.id = D.invoiceId  $whrQry GROUP BY I.id ORDER BY I.id");
                  ...
                  ?>
                      <tr>
                        ...
                        <td><?php echo $recieptInitial.'-'.$result->id; ?></td>
