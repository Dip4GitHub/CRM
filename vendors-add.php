...
$pageTitle      = 'vendors';
$tableName      = generateSql('#__vendors');
$currentPage    = 'vendors';
...
$vendorsType    = isset($_REQUEST['vendorsType']) ? $_REQUEST['vendorsType'] : '1';


if ($submit == 'Add' or $submit == 'Update') {
  $commonQry  = "`name`='$name', `email`='$email', `mobile`='$mobile', `gender`='$gender', `activeStatus`='$activeStatus', `companyName`='$companyName', `address`='$address', `city`='$city', `pincode`='$pincode', `gstNumber`='$gstNumber', `state`='$state', `adharNo`='$adharNo', `panNo`='$panNo', `vendorsType`='$vendorsType'";
  ...
  if ($submit      == 'Add') {
    if ($errmsg == '') {
      ...
      if ($success) {
        $description = "Vendor $name is created by $guserName on" . date("l jS \of F Y h:i:s A");
        $logQuery      = "INSERT INTO crm_logtable SET `idType`='4', `useId`='$id', `name`= 'Vendor $name is created', `description` ='$description', `createdId`='$guserId', `createdDate`=NOW()";
    ...
    if ($errmsg == '') {
      ...
      if ($success) {
        $description = "Vendor $name is Updated by $guserName on" . date("l jS \of F Y h:i:s A");
        $logQuery      = "INSERT INTO crm_logtable SET `idType`='4', `useId`='$id', `name`= 'Vendor $name is Updated', `description` ='$description', `createdId`='$guserId', `createdDate`=NOW()";
        $success    = mysqli_query($Dbconnect, $logQuery);
        location("$currentPage.php?msg=$msg");
      } else {
        $errmsg   = "Error: " . mysqli_error($Dbconnect);
              ...
              <form name="frm" method="post" action="<?php echo $currentPage; ?>-add.php" autocomplete=off>
                <div class="grid grid-cols-12 gap-1.5">
                  ...
                  <div class="col-span-12 sm:col-span-6 my-1">
                    <label for="name" class="col-form-label">Vendor Type</label><span style="color:red"> *</span>
                    <?php echo displaySelect('vendorsType', $vendorsTypeArray, $vendorsType, 'class="form-control"') ?>
                  </div>
                  ...
                  <div class="col-span-12 sm:col-span-6 my-1">
                    ...
                    <input class="form-control" type="text" id="city" name="city" placeholder="City" value="<?php echo $city; ?>" value="<?php echo $city; ?>" required>
                  </div>
                  <div class="col-span-12 sm:col-span-6 my-1">
