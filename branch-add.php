...
$pageTitle      = 'Branch';
$tableName      = generateSql('#__branch');
$currentPage    = 'branch';
...
$billStart      = isset($_POST['billStart']) ? $_POST['billStart'] : 'NGP';
$address        = isset($_POST['address']) ? $_POST['address'] : '';
$activeStatus   = isset($_POST['activeStatus']) ? $_POST['activeStatus'] : '1';
$branchType     = isset($_POST['branchType']) ? $_POST['branchType'] : '';


if ($submit == 'Add' or $submit == 'Update') {
  $commonQry  = "`name`='$name', `billStart`='$billStart', `activeStatus`='$activeStatus', `address`='$address', `branchType`='$branchType'";
  ...
  if ($submit      == 'Add') {
    if ($errmsg == '') {
      ...
      if ($success) {
        $description = "Branch $name is created by $guserName on" . date("l jS \of F Y h:i:s A");
        $logQuery      = "INSERT INTO crm_logtable SET `idType`='4', `useId`='$id', `name`= 'Branch $name is created', `description` ='$description', `createdId`='$guserId', `createdDate`=NOW()";
    ...
    if ($errmsg == '') {
      ...
      if ($success) {
        $description = "Branch $name is Updated by $guserName on" . date("l jS \of F Y h:i:s A");
        $logQuery      = "INSERT INTO crm_logtable SET `idType`='4', `useId`='$id', `name`= 'Branch $name is Updated', `description` ='$description', `createdId`='$guserId', `createdDate`=NOW()";
        $success    = mysqli_query($Dbconnect, $logQuery);
        location("$currentPage.php?msg=$msg");
      } else {
        $errmsg   = "Error: " . mysqli_error($Dbconnect);
            ...
            <div class="card-body">
              <form name="frm" method="post" action="<?php echo $currentPage; ?>-add.php" autocomplete=off>
                <div class="grid grid-cols-12 gap-1.5">
                  ...
                  <div class="col-span-12 sm:col-span-6 my-1">
                    <label for="billStart" class="col-form-label">Bill Initial</label><span style="color:red"> *</span>
                    <input class="form-control" type="text" id="billStart" name="billStart" value="<?php echo $billStart; ?>" required>
                  </div>
                  <div class="col-span-12 sm:col-span-6 my-1">
                    <label for="branchType" class="col-form-label">Branch Type</label><span style="color:red"> *</span>
                    <?php displaySelect('branchType', $branchTypeArray, $branchType, 'class="form-control" required'); ?>
                  </div>
                  <div class="col-span-12 sm:col-span-6 my-1">
