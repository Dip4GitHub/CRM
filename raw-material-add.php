...
if ($submit == 'Add' or $submit == 'Update') {
  $commonQry  = "`name`='$name', `productType`='2', `unit`='$unit', `gstType`='$gstType', `activeStatus`='$activeStatus', `hsncode`='$hsncode', `gst`='$gst', `salePrice`='$price'";
  ...
  if ($submit      == 'Add') {
    ...
    $msg        = 'New Raw Material Product Created successfully.';
    if ($success) {
      $description = "Raw Material Product $name is created by $guserName on" . date("l jS \of F Y h:i:s A");
      $logQuery      = "INSERT INTO crm_logtable SET `idType`='5', `useId`='$id', `name`= 'Raw Material Product $name is created', `description` ='$description', `createdId`='$guserId', `createdDate`=NOW()";
    ...
    if ($errmsg == '') {
      ...
      $success    = mysqli_query($Dbconnect, $query);
      $msg        = 'Raw Material Product Updated successfully.';
      if ($success) {
        $description = "Raw Material Product $name is Updated by $guserName on" . date("l jS \of F Y h:i:s A");
