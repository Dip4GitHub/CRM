...
if ($submit == 'Add' or $submit == 'Update') {
  $commonQry  = "`salePurchaseDate`='$salePurchaseDate', `customerName`='$customerName', `customerId`='$customerId', `invoiceNo`='$invoiceNo', `finalTotal`='$finalTotal', `branchId`='$BranchID'";
  ...
  if ($submit      == 'Add') {
    if ($errmsg == '') {
      ...
      $msg        = 'New Sale/Order Created successfully.';

      if ($success) {
        $description = "Sale/Order #$id of Customer $customerName is created by $guserName on" . date("l jS \of F Y h:i:s A");
