...
if ($submit == 'Add' or $submit == 'Update') {
  $commonQry  = "`name`='$name', `email`='$email', `mobile`='$mobile', `gender`='$gender', `activeStatus`='$activeStatus', `companyName`='$companyName', `address`='$address', `city`='$city', `pincode`='$pincode', `state`='$state'";
  ...
  if ($submit      == 'Add') {
    if ($errmsg == '') {
      ...
      if ($success) {
        $description = "Lead $name is created by $guserName on" . date("l jS \of F Y h:i:s A");
        $logQuery      = "INSERT INTO crm_logtable SET `idType`='2', `useId`='$id', `name`= 'Lead $name is created', `description` ='$description', `createdId`='$guserId', `createdDate`=NOW()";
    ...
    if ($errmsg == '') {
      ...
      if ($success) {
        $description = "Lead $name is Updated by $guserName on" . date("l jS \of F Y h:i:s A");
        $logQuery      = "INSERT INTO crm_logtable SET `idType`='1', `useId`='$id', `name`= 'Lead $name is Updated', `description` ='$description', `createdId`='$guserId', `createdDate`=NOW()";
        $success    = mysqli_query($Dbconnect, $logQuery);
        location("$currentPage.php?msg=$msg");
      } else {
        $errmsg   = "Error: " . mysqli_error($Dbconnect);
