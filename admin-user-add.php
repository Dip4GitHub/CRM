...
$userType       = isset($_REQUEST['userType']) ? $_REQUEST['userType'] : '2';
$branchId       = isset($_REQUEST['branchId']) ? $_REQUEST['branchId'] : '';
$branchType     = isset($_REQUEST['branchType']) ? $_REQUEST['branchType'] : '';

$password       = isset($_REQUEST['password']) ? $_REQUEST['password'] : '';
$cnfmPassword   = isset($_REQUEST['cnfmPassword']) ? $_REQUEST['cnfmPassword'] : '';
$passwordQry = '';
if ($id > 0) {
  $submitMsg  = "Update";
} else {
...
if ($submit      == 'Add' or $submit   == 'Update') {
  ...
  if ($submit      == 'Add') {
    if ($password == '' or $cnfmPassword == '') {
      $errmsg = "Please Enter Password And Confirm it First.";
    }

    if ($password == $cnfmPassword) {
      $password = MD5Encrypt($password);
      $passwordQry = "`password`='$password',";
    }
    $commonQry  = "$passwordQry `name`='$name', `email`='$email', `mobile`='$mobile', `gender`='$gender', `activeStatus`='$activeStatus', `userType`='$userType', `branchType`='$branchType',";
    ...
    if ($errmsg == '') {
      ...
      if ($success) {
        $logQuery      = "INSERT INTO crm_logtable SET `idType`='3', `useId`='$id', `name`= '$name is created', `description` ='$name is created by $guserName', `createdId`='$guserId', `createdDate`=NOW()";
    ...
    if ($password != '' and $cnfmPassword != '') {
      $password = MD5Encrypt($password);
      $passwordQry = "`password`='$password',";
    }

    $commonQry  = "$passwordQry `name`='$name', `email`='$email', `mobile`='$mobile', `gender`='$gender', `activeStatus`='$activeStatus', `userType`='$userType',`branchId`='$branchId', `branchType`='$branchType'";

    if ($errmsg == '') {
      $query      = "UPDATE `$tableName` SET $commonQry WHERE `id`='$id'";
      $success    = mysqli_query($Dbconnect, $query);
      $msg        = 'User Updated successfully.';
      if ($success) {
        $logQuery      = "INSERT INTO crm_logtable SET `idType`='3', `useId`='$id', `name`= '$name is Updated', `description` ='$name is Updated by $guserName', `createdId`='$guserId', `createdDate`=NOW()";
        $success    = mysqli_query($Dbconnect, $logQuery);
        location("$currentPage.php?msg=$msg");
      } else {
        $errmsg   = "Error: " . mysqli_error($Dbconnect);
...
<body>
          ...
          <div class="card">
            ...
            <div class="card-body">
              <form name="frm" method="post" action="<?php echo $currentPage; ?>-add.php" autocomplete=off>
                ...
                <input type="hidden" name="branchType" value="<?php echo $branchType; ?>">
                ...
                <div class="grid grid-cols-12 gap-1.5">
                  ...
                  <div class="col-span-12 sm:col-span-6 my-1">
                    ...
                    <?php displaySelect('gender', $genderArray, $gender, 'class="form-control"'); ?>

                  </div>

                  <div class="col-span-12 sm:col-span-6 my-1">
                    <label for="password" class="col-form-label">Password</label><span style="color:red"> *</span>
                    <input class="form-control" type="password" name="password" id="password" placeholder="Password">
                  </div>
                  <div class="col-span-12 sm:col-span-6 my-1">
                    <label for="cnfmPassword" class="col-form-label">Confirm Password</label><span style="color:red"> *</span>
                    <input onblur="checkPass(this.value);" class="form-control" type="text" id="cnfmPassword" name="cnfmPassword" placeholder="Confirm Password">
                  </div>

                  <div class="col-span-12 sm:col-span-6 my-1">
                    <label for="activeStatus" class="col-form-label">Status</label><span style="color:red"> *</span>
                    <?php displaySelect('activeStatus', $statusArray, $activeStatus, 'class="form-control"'); ?>
                  </div>
                  <div class="col-span-12 sm:col-span-6 my-1">
                    <label for="userType" class="col-form-label">User Type</label><span style="color:red"> *</span>
                    <?php displaySelect('userType', $userTypeArray, $userType, 'class="form-control"'); ?>
                  </div>
                  <div class="col-span-12 sm:col-span-6 my-1">
                    <label for="branchId" class="col-form-label">Branch</label>
                    <?php
                    $branchIdArray  = getArrayfromTable('id', 'name', '#__branch', ' WHERE activeStatus=1 ORDER BY name ASC');
                    displaySelect('branchId', $branchIdArray, $branchId, 'class="form-control" onchange="getBranchType(this.value)"'); ?>
                  </div>
                  <div class="col-span-12 sm:col-span-6 my-1">
                    <label for="branchType" class="col-form-label">Branch Type</label>
                    <div id="branchType">
                      <?php
                      displaySelect('branchType', $branchTypeArray, $branchType, 'class="form-control""'); ?>
                    </div>
                  </div>
  ...
  <script>
    function checkPass(cnfmPassword) {
      var password = $('#password').val();
      if (cnfmPassword != password) {
        alert("Your Password And Confirm Password Not Match, Please make sure Both Matches.");
      }
    }

    function getBranchType(branchId) {
      $.post('includes/ajax/getBranchType.php', {
          branchId: branchId
        },
        function(data, status) {
          console.log(status)
          $('#branchType').html(data);
        });
    }
  </script>
